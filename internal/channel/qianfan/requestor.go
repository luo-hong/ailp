...
func (r *Requestor) addAuthInfo(ctx context.Context, request *QfRequest) error {
	...
	log.Print("no enough credential found. Please check whether (ak, sk) or (access_key, secret_key) is set in config")
...
func (r *Requestor) sign(request *QfRequest) error {
	...
	if port == "" {
		if u.Scheme == "http" {
			...
			log.Printf("Got unexpected protocol `%s` in requested API url `%s`.", u.Scheme, request.URL)
...
func (si *streamInternal) Recv(resp QfResponse) error {
	...
	if runErr != nil {
		log.Printf("Stream operation cancelled due to context, error: %s", runErr)
		return runErr
	}
	return err
}
