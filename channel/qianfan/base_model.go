...
func (s *ModelResponseStream) checkResponseError(ctx context.Context) error {
	...
	for retryCount := 0; retryCount < s.Options.LLMRetryCount || s.Options.LLMRetryCount == 0; retryCount++ {
		...
		if strings.Contains(contentType, "application/json") {
			...
			log.Printf("stream request got error: %s, retrying request... retry count: %d", apiError, retryCount)
		} else {
			return nil
		}
		time.Sleep(
...
func (m *BaseModel) withRetry(fn func() error) error {
	...
	for retryCount := 0; retryCount < m.Options.LLMRetryCount || m.Options.LLMRetryCount == 0; retryCount++ {
		...
		log.Printf("request got error: %s, retrying request... retry count: %d", err, retryCount)
		time.Sleep(
			time.Duration(
				math.Pow(
					2,
