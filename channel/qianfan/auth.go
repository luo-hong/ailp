...
func (m *AuthManager) GetAccessToken(ctx context.Context, ak, sk string) (string, error) {
	...
	log.Printf("Access token of ak `%s` not found, tring to refresh it...", maskAk(ak))
...
func (m *AuthManager) GetAccessTokenWithRefresh(ctx context.Context, ak, sk string) (string, error) {
	...
	if ok {
		...
		if current.Sub(lastUpdate) < time.Duration(GetConfig().AccessTokenRefreshMinInterval)*time.Second {
			log.Printf("Access token of ak `%s` was freshed %s ago, skip refreshing", maskAk(ak), current.Sub(lastUpdate))
			return token.token, nil
		}
	}
	...
	if resp.Error != "" {
		log.Printf("refresh access token of ak `%s` failed with error: %s", maskAk(ak), resp.ErrorDescription)
		return "", &APIError{Msg: resp.ErrorDescription}
	}
	log.Printf("refresh access token of ak `%s` success", maskAk(ak))
